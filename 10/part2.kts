...
val outputBins: MutableMap<Int, Int> = mutableMapOf()
...
while (factory.filterValues { it.canProceed() }.count() > 0) {
    factory.filterValues { it.canProceed() }.forEach { _, v ->
        ...
        when (lowResult.disposition) {
            ...
            is Disposition.OutputBin -> outputBins.put(lowResult.disposition.binNumber, lowResult.value)
        }

        when (highResult.disposition) {
            is Disposition.Bot -> passChipToBot(highResult.disposition.botNumber, highResult.value)
            is Disposition.OutputBin -> outputBins.put(highResult.disposition.binNumber, highResult.value)
        }
    }
}

println(outputBins.getValue(0) * outputBins.getValue(1) * outputBins.getValue(2))